...
func (s *Service) StartProject(projectID string) error {
	...
	validateProjectTransitionOrWarn(project.Status, ProjectStatusInProgress, projectID)
	if err := s.db.UpdateProjectStatus(ctx, projectID, ProjectStatusInProgress); err != nil {
...
func (s *Service) StopProject(projectID string) {
	project, err := s.db.GetProject(context.Background(), projectID)
	if err == nil {
		validateProjectTransitionOrWarn(project.Status, ProjectStatusCancelled, projectID)
	}
	_ = s.db.UpdateProjectStatus(context.Background(), projectID, ProjectStatusCancelled)
}
...
func (s *Service) IsRunning(projectID string) bool {
	...
	return project.Status == ProjectStatusInProgress
}
