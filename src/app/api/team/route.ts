...
// GET: list all employees of the single company
export async function GET() {
  try {
    ...
    const employees = await prisma.employee.findMany({
      where: { companyId },
      include: {
        role: { include: { skills: { include: { skill: { select: { id: true, name: true } } } } } },
      },
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to list team" },
      { status: 500 }
    );
  }
}

// POST: hire a new employee for the single company
